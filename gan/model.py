...
class UpConvLayer(nn.Sequential):
    def __init__(self, in_channels, out_channels, activation, **kwargs):
...
class Generator(nn.Sequential):
    def __init__(self, n_z, n_layers=5, out_channels=3):
        self.n_z = n_z
        ...
        for _ in range(n_layers):
            layers.append(UpConvLayer(prev_filt, int(prev_filt / 2), activation='leakyrelu',
                                      kernel_size=(6, 6), stride=(2, 2), padding=2))
            prev_filt = int(prev_filt / 2)

        super().__init__(
            nn.Linear(n_z, 8 * 8 * 512),
            nn.LeakyReLU(0.2, True),
            Rearrange('b (h w z) -> b z h w', h=8, w=8, z=512),
            *layers,
